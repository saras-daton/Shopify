...
{% for i in results_list %}
    ...
    FROM (
        ...
        Dense_Rank() OVER (PARTITION BY a.id, note_attributes.VALUE:name order by a.{{daton_batch_runtime()}} desc) row_num,
        {% else %}
        ...
        Dense_Rank() OVER (PARTITION BY a.id, note_attributes.name order by a.{{daton_batch_runtime()}} desc) row_num,
        {% endif %}
        email,
        estimated_taxes,
        financial_status,
