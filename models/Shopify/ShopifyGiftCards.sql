...
{{set_table_name('%shopify%gift_cards')}} and lower(table_name) not like '%googleanalytics%' and lower(table_name) not like 'v1%'
{% endset %}  
...
{% for i in results_list %}
    ...
    {% else %}
        ...
        note,
        template_suffix,
        cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="updated_at") }} as {{ dbt.type_timestamp() }}) as expires_on,
        last_characters,
