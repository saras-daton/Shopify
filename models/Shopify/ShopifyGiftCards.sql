...
{{set_table_name('%shopify%gift_cards')}}
{% endset %}  
...
{% for i in results_list %}
    ...
    {% endif %}
        ...
        currency,
        initial_value,
        safe_cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="disabled_at") }} as {{ dbt.type_timestamp() }}) as disabled_at,
        safe_cast(line_item_id as string) as line_item_id,
        safe_cast(api_client_id as string) as api_client_id,
        safe_cast(user_id as string) as user_id,
        safe_cast(customer_id as string) as customer_id,
        note,
        template_suffix,
        safe_cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="updated_at") }} as {{ dbt.type_timestamp() }}) as expires_on,
        last_characters,
