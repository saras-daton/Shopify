...
{% for i in results_list %}
    ...
    {% endif %}
        ...
        '{{brand}}' as brand,
        '{{store}}' as store,
        coalesce(safe_cast(a.id as string),'n/a') as customers_id,
        email,
        accepts_marketing,
        safe_cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="safe_cast(a.created_at as timestamp)") }} as {{ dbt.type_timestamp() }}) as created_at,
        safe_cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="safe_cast(a.updated_at as timestamp)") }} as {{ dbt.type_timestamp() }}) as updated_at,
        a.first_name,
        a.last_name,
        orders_count,
        a.state,
        ...
        a.currency,
        a.phone,
        safe_cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="accepts_marketing_updated_at") }} as {{ dbt.type_timestamp() }}) as accepts_marketing_updated_at,
        ...
        safe_cast(last_order_id as string) last_order_id,
        last_order_name,
