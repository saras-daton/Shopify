...
{{ set_table_name('%shopify%price_rules') }}
{% endset %}
...
{% for i in results_list %}
    ...
    select 
        ...
        allocation_method,
        once_per_customer,
        safe_cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="starts_at") }} as {{ dbt.type_timestamp() }}) as starts_at,
        safe_cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="ends_at") }} as {{ dbt.type_timestamp() }}) as ends_at,
