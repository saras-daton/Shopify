...
{% for i in results_list %}
    ...
    {% endif %}
        ...
        select 
            ...
            type,
            test,
            safe_cast(payout_id as string) as payout_id,
            payout_status,
            currency,
            safe_cast(amount as numeric) as amount,
            safe_cast(fee as numeric) as fee,
            safe_cast(net as numeric) as net,
            safe_cast( source_id as string) as source_id,
            source_type,
            safe_cast({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="safe_cast(processed_at as timestamp)") }} as {{ dbt.type_timestamp() }}) as processed_at,
            safe_cast(source_order_id as string) as source_order_id,
            safe_cast(source_order_transaction_id as string) as source_order_transaction_id,
            {% if var('currency_conversion_flag') %}
