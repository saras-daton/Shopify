...
{% set result =set_table_name('shopify_transactions_tbl_ptrn','%shopify%transactions','shopify_transactions_exclude_tbl_ptrn','%tender%, %balance%') %}
...
{% for i in result %}
    ...
    '{{env_var("DBT_CLOUD_RUN_ID", "manual")}}' as _run_id
        ...
        qualify row_number() over (partition by id order by a.{{daton_batch_runtime()}} desc) = 1
